...
*   **개발/테스트 환경:** 현재 프로젝트는 테스트 단계로, 일부 API 엔드포인트에서 인증을 임시로 비활성화하여 개발 및 테스트를 용이하게 합니다.
*   **운영 환경 전환 시 주의사항:**
    *   모든 API 엔드포인트에 `IsAuthenticated` 권한을 적용해야 합니다.
    *   하드코딩된 인증 정보는 반드시 제거하고 Django 인증 시스템을 사용해야 합니다.
    *   민감한 정보(비밀번호, API 키 등)는 환경 변수로 관리해야 합니다.
*   **보안 체크리스트:**
    *   [ ] 모든 ViewSet에 `permission_classes = [IsAuthenticated]` 적용
    *   [ ] 하드코딩된 인증 로직 제거
    *   [ ] 환경 변수를 통한 민감 정보 관리
    *   [ ] HTTPS 적용
    *   [ ] CSRF 보호 활성화
